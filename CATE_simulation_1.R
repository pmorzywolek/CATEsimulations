...
#################################################
# R-code to replicate the simulation set-up 1 from the paper
...
p = 20 # Dimension of covariates X
sigma = 0.5

# Select list of models to be included in the SuperLearner
SL.library = c("SL.glm", "SL.ranger", "SL.glmnet", "SL.xgboost")

# Simulation set-up 1 is based on the simulation set-up C from (Nie and Wager, 2021)
get.params = function() {
  X = matrix(rnorm(n * p), n, p)
  b = 2 * log(1 + exp(X[, 1] + X[, 2] + X[, 3] + X[, 4] + X[, 5]))
  e = 1 / (1 + exp(X[, 2] + X[, 3] + X[, 4] + X[, 5]))
  tau = rep(1, n)
...
for (i in 1:nsim) {
  ...
  # Y is the outcome, X are covaraites and A is the exposure.
  #####################################
  ...
  #####################################
  # Compute the pseudooutcomes for different learners in the training sets.
  #####################################
  ...
  data_train_A$pseudooutcome_R <- 
    (data_train_A$y - data_train_A$ps_SL * data_train_A$Q1 - (1 - data_train_A$ps_SL) * data_train_A$Q0) / (data_train_A$a - data_train_A$ps_SL)
  data_train_B$pseudooutcome_R <-
    (data_train_B$y - data_train_B$ps_SL * data_train_B$Q1 - (1 - data_train_B$ps_SL) * data_train_B$Q0) / (data_train_B$a - data_train_B$ps_SL)
